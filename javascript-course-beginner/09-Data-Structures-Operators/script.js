...
const timeGameEvents = [...gameEvents.keys()].pop();
console.log(timeGameEvents);
console.log(
  `An event happened, on average, every ${
    timeGameEvents / gameEvents.size
  } minutes`
);
...
document.querySelector('button').addEventListener('click', function () {
  const textName = document.querySelector('textarea').value;
  const rows = textName.split('\n');
  for (const [i, row] of rows.entries()) {
    const [first, second] = row.toLowerCase().trim().split('_');
    const outputs = `${first}${second.replace(
      second[0],
